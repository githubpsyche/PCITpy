...
def common_to_all_curves(curve_type, get_info, *varargin):
    ...
    if get_info == 'initial_sampling':
        ...
        if input_params == 'con':
            ...
            th_con_params_indices = th_con_params_indices[np.random.permutation(np.shape(th_con_params_indices)[0])] # Randomly permuting the th_con trial indices
            out = params[th_con_params_indices[0], :] # picking one consistent particle
        elif input_params == 'inc':
            th_inc_params_indices = np.where(np.logical_not(params_indices)) # Finding the theory inconsistent trial indices
            if len(th_inc_params_indices) <= 0:
                raise ValueError('Did not generate any theory inconsistent indices!')
            th_con_params_indices = th_con_params_indices[np.random.permutation(np.shape(th_con_params_indices)[0])] # Randomly permuting the th_inc trial indices
